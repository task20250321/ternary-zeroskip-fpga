...
// Dense ternary baseline with PE-level output ownership.
//
// Compared with ternary_dense_lockstep_accelerator:
//   - removes NUM_PE-input reduction tree
//   - removes shared output-psum RAM write path
//   - PE p owns global 5-trit groups g with g mod NUM_PE == p
//   - each PE processes all inputs for its owned outputs
//   - every ternary coordinate is processed (no zero skipping)
//   - private psum banks are drained in global-output order at layer end
module ternary_dense_owned_accelerator #(
    ...
    localparam integer FINAL_COUNT_WIDTH =
        (OUT_FEATURES <= 0) ? 1 : $clog2(OUT_FEATURES + 1);
    ...
    dense_owned_private_accumulator #(
        .NUM_PE          (NUM_PE),
    ...
    always_ff @(posedge clk_i) begin
        if (rst_i || run_reset || (state_q != ST_FINAL_DRAIN)) begin
            ...
            if (final_issue) begin
                ...
                if (final_scan_coord_q == 3'd4) begin
                    final_scan_coord_q <= '0;
...
`ifndef SYNTHESIS
    initial begin
        if (DDR_WORD_BITS != 256)
            $fatal(1, "Dense-owned baseline expects DDR_WORD_BITS=256");
        if (NUM_PE < KEYS_PER_WORD || (NUM_PE % KEYS_PER_WORD) != 0)
            $fatal(1, "NUM_PE must be a multiple of 32");
        if (!(NUM_PE == 32 || NUM_PE == 64 || NUM_PE == 128))
            $fatal(1, "Evaluated Dense-owned PE counts are PE32/PE64/PE128");
        if (LUT_IMPL != 0)
            $fatal(1, "Dense-owned baseline currently uses combinational dense LUT only");
    end
`endif
endmodule
